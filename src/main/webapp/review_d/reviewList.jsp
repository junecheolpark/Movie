				...
				<div class="container-fluid">
					...
					<div class="collapse navbar-collapse justify-content-end" id="navbarNavDropdown">
						...
						<form class="searchForm d-flex" method="get" action="/search.movie">
							<input class="searchInput form-control me-2" type="search" name='val' placeholder="Search" aria-label="Search"> <input
				...
				<div class="row w-100 align-items-center">
					...
					<div class="col-5">
						<div class="row">
							...
							<div class="col-2">
								<a href="/wishlist.wish" class="align-items-center "> <img class="img-fluid" id="cartIcon" src="/images/찜.png">
									<!-- <p class="text-light" id="cart">찜한 영화</p> -->
								</a> <a href="/Mypage/mypageIndex.jsp" class="align-items-center"> <img class="img-fluid" id="myPageIcon"
									src="/images/마이페이지.png"> <!-- <p class="text-light" id="myPage">마이페이지</p> -->
								</a>
							</div>
							<div class="col-5">
								<form class="searchForm d-flex" method="get" action="/search.movie">
									<input class="searchInput form-control me-2" type="search" placeholder="Search" aria-label="Search" name='val'> <input
										type="hidden" name='curPage' value="1"> <input type="hidden" name='s_type' value="movieNm">
									<button class="searchBtn btn btn-outline-success" type="submit">Search</button>
								</form>
							</div>
											...
											<div class="productDiv1">
												...
												<div class="productAvgPoint">
													...
													<div class="avgPointNum" data-value='${movie.value}'>
														<fmt:formatNumber value="${movie.value['avg']}" type="pattern" pattern="0.00" />
														(${movie.value['count']})
													</div>
												</div>
											</div>
									...
									<div class="reviewLike">
										<c:forEach items="${likes}" var="like">
											<c:if test="${reviewDTO.seq_review eq like.key}">
												...
												<button type='button' class='likeBtn' value="${reviewDTO.seq_review}">좋아요</button>
												<span class="hateSpan">${like.value['hate']}</span>
												<button type="button" class="hateBtn" value="${reviewDTO.seq_review}">싫어요</button>
											</c:if>
										</c:forEach>
									</div>
								</div>
		...
		<nav aria-label="PageNav">
			<ul class="pagination d-flex justify-content-center">
				<c:if test="${empty s_type}">
					<li class="page-item prevBtn"><a class="page-link" href="/toReviewList.re?curPage=${hashMap.naviStart-1}"
						data-value="${hashMap.naviStart-1}">Previous</a></li>
					<c:forEach var="pageNum" begin="${hashMap.naviStart}" end="${hashMap.naviEnd}" step="1">
						<li class="page-item"><a class="page-link" href="/toReviewList.re?curPage=${pageNum}" data-value="${pageNum}">${pageNum}</a>
						</li>
					</c:forEach>
				...
				<c:if test="${s_type eq 'genreAlt'}">
					<li class="page-item prevBtn"><a class="page-link"
						href="/show.re?s_type=${s_type}&curPage=${hashMap.naviStart-1}&val=${val}" data-value="${hashMap.naviStart-1}">Previous</a>
					</li>
					<c:forEach var="pageNum" begin="${hashMap.naviStart}" end="${hashMap.naviEnd}" step="1">
						<li class="page-item"><a class="page-link" href="/show.re?s_type=${s_type}&curPage=${pageNum}&val=${val}"
							data-value="${pageNum}">${pageNum}</a></li>
					</c:forEach>
					<li class="page-item nextBtn"><a class="page-link"
						href="/show.re?s_type=${s_type}&curPage=${hashMap.naviEnd+1}&val=${val}" data-value="${hashMap.naviEnd+1}">Next</a></li>
				</c:if>
			</ul>
		</nav>
	</div>
		...
		<div class="container">
			<div class="row">
				...
				<div class="col-4 offset-1">
					...
					<div class="snsBox">
						<div class="snsIcon1">
							<a href="https://www.kakaocorp.com/" class="d-flex align-items-center mb-2 mb-lg-0"> <img id="kakaoIcon" src="/images/">
							</a>
						</div>
						<div class="snsIcon2">
